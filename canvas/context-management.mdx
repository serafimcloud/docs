...
- **On‑canvas items**: any shape already on the canvas (prototypes, components, inspiration screens, images)
...
<Steps>
  ...
  <Step title="Inline mentions with @">
    In the chat, type `@` to open the inline menu. Start typing to search by name — the AI instantly searches through file names, frame names, prototypes, and references as you type.
  </Step>
</Steps>
...
<Tip>
**Explore & Merge**: Explore multiple prototypes, then add another prototype as context to merge the best parts into one result.
</Tip>
