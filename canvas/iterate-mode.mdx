...
Each variant (frame) can have multiple chats in Iterate mode. Click the **+** button in the chat header to create a new chat for the same variant:

- **Multiple chats** — Create separate conversations for different aspects of the same feature
- **Chat history** — View and switch between all chats for a variant in the chat history panel
- **Isolated contexts** — Each chat maintains its own conversation context
- **Forking** — Duplicate a variant to keep all chat histories when exploring alternatives
