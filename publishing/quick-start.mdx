...
dependencies, builds the component, and then opens a Studio draft on the
**Component** tab. Review the live preview in light and dark themes, test its
states, capture and upload the required cover, then finish publishing in
Studio. The CLI waits and prints the component URL only after that final step.

Add `--to <library-slug>` to target a team library, or use
`--visibility private` for a Private component. The default is Published. A
`21st login` session or API key can create a component draft; editing and
deleting require a `21st_sk_…` key from
[Settings → API keys](https://21st.dev/settings/api-keys).

An existing owned slug is updated directly. Use `--no-open` to print the draft
URL without opening Studio, or <kbd>Ctrl</kbd>+<kbd>C</kbd> to stop waiting
without deleting the draft.

New CLI drafts currently accept one self-contained component file and one demo.
Additional local files, `21st.json`, and registry dependencies are not supported
for a new slug yet.
