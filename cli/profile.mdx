...
Showcase blocks for your published ASCII recipes, templates, themes, and
gradients use `{ "type": "gradient", "refId": "..." }` (replace `gradient`
with the matching content type). `profile get` prints these references as
`ref:<id>` in its human-readable output.
