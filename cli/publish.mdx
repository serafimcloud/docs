...
dependency-checked, installed, and built, then the CLI waits for the required
cover before opening the draft in the dedicated **CLI Review** — the browser
gets a component that is ready to publish, not a build in progress. An optional
video keeps generating in the background and is included only if it finishes.
