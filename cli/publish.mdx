...
```bash
21st render ./Component.tsx --demo ./Component.demo.tsx --out ./preview
21st render ./Component.tsx --video --json
```
...
```bash
21st library
21st library show sophon
21st library create design-system --name "Design System"
21st library update sophon --logo ./logo.svg --display-name "Sophon" --public
```
...
`--private` for the library's public page. Logos accept PNG, JPEG, WebP or
SVG up to 2 MiB. Add `--json` to any of them for machine-readable output.
...
dependency-checked, installed, and built before its draft opens in the
dedicated **CLI Review**. Studio then generates the required cover and attempts
an optional video in the background; publishing stays locked until every demo
has a cover, but never waits for video. A cover you attach with
...
```bash
# Fix Component.tsx, then push it into the draft you already have.
21st publish ./Component.tsx \
  --description "A responsive pricing table" \
  --draft draft:<uuid> --prepare
```

The draft re-signs its revision and captures the cover and video again, exactly
as an edit made in the review screen does. Anything the new code imports is
installed into the draft first, so adding a dependency between runs works.
Media you uploaded yourself is kept. A push carries the whole set: repeat
...
| `--demo <file>`                       | Use a specific demo file (repeat for more) |
| `--preview <img>`                     | Use this image as the demo's cover         |
...
| `--draft draft:<uuid>`                | Push this file into that draft and resume  |
| `--no-push`                           | Resume without replacing the draft's source |

See [Publishing → Quick Start](/publishing/quick-start) for the full flow and requirements.

### Controls come from the demo file

A demo that declares a module-level `const settings = { … }` and takes it as
...
[Demo Controls](/publishing/demo-controls).
...
| `cli_draft_waking`                        | The draft's workspace was still waking up       | Nothing: the CLI waits and sends the source again                    |
| `database_busy`                           | Our database was busy for a moment              | Nothing: the CLI retries after `Retry-After` seconds                 |
